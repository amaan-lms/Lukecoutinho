...
import { 
  ...
  ArrowRight, Activity, Dumbbell, Moon, 
  Apple, HeartPulse, CheckCircle2
} from 'lucide-react';
import img from '../../../assets/10.jpg'; 

const TestosteroneCourseView = () => {
  const [isHovered, setIsHovered] = useState(false);

  const courseStats = [
    { icon: <Users size={18} />, label: "27 Students Enrolled", detail: "Hormone health course" },
  ...
  const learningObjectives = [
    { 
      ...
      desc: "Learn its role in energy, libido, muscle, and overall health." 
    },
    { 
      icon: <Apple />, 
      title: "Nutrition & Fasting", 
      desc: "Use diet and mindful fasting to support hormone balance." 
    },
    { 
      icon: <Dumbbell />, 
      title: "Exercise for Hormones", 
      desc: "Discover workouts that naturally support testosterone levels." 
    },
    { 
      icon: <Moon />, 
      title: "Sleep & Recovery", 
      desc: "Improve sleep quality to optimize hormone production." 
    }
  ];
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 h-full flex flex-col justify-end pb-12">
          <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="space-y-4">
            <span className="bg-orange-600 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em] shadow-xl shadow-orange-500/20">
              Hormonal Health Course
            </span>
            <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter max-w-5xl leading-[0.9]">
              Testosterone Boost <br/>
              <span className="text-orange-500 italic font-light">Healthy Hormone Balance</span>
            </h1>
          </motion.div>
        </div>
      </section>
        ...
        <div className="lg:col-span-8 space-y-16">
          ...
          <section className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <h2 className="text-2xl font-bold mb-6 text-slate-800 flex items-center gap-3">
              <Activity className="text-orange-500" /> Boost Energy & Vitality Naturally
            </h2>
            <p className="text-slate-500 leading-relaxed text-lg italic">
              Learn how lifestyle habits influence testosterone and overall hormonal balance. 
              Discover practical strategies to improve energy, libido, muscle tone, and fertility — naturally.
            </p>
          </section>
          ...
          <section className="bg-slate-900 p-12 rounded-[3rem] text-white overflow-hidden relative shadow-2xl">
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                {[
                  "People feeling low energy or hormonal imbalance",
                  "Individuals wanting to improve libido and vitality",
                  "Those exploring natural fertility support",
                  "Anyone aiming for better performance and recovery"
                ].map((text, i) => (
          ...
          <section className="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden">
            <div className="p-10 space-y-6">
              ...
              <div className="space-y-3">
                <p className="text-sm text-slate-500 flex gap-2"><CheckCircle2 size={16} className="text-orange-500" /> No prior hormone knowledge required</p>
                ...
                <p className="text-sm text-slate-500 flex gap-2"><CheckCircle2 size={16} className="text-orange-500" /> Practical lifestyle-based guidance</p>
              </div>
              <p className="text-slate-400 text-[11px] leading-relaxed pt-6 border-t border-slate-50 italic">
                Educational use only. Consult a qualified healthcare professional for medical advice.
              </p>
            </div>
          </section>
        </div>
...
export default TestosteroneCourseView;
